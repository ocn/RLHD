...
- Task 1 baseline `R-20260804-008`: exact mapped-uploader hashes, 18-vertex BASE geometry, a 52x52 footprint count, two RGB samples at tolerance 2, exact background/alpha/UI samples, full validation scope, and post-close handle balance passed. Review then strengthened this into the pinned all-pixel coverage/composition oracle; a new committed-oracle run is required before closing the checkpoint. OpenGL full-image equivalence remains unverified.
