...
| Vulkan control spike | `spikes/vulkan-control`, Task 2 `MacMetalSurface`, Task 3 `SyntheticUi`, LWJGL 3.3.2 Java bindings, external Vulkan loader/MoltenVK ICD | Opt-in Java-only control artifact. Surface remains Task 2-owned; the renderer borrows its layer until Vulkan teardown, owns all Vulkan objects, and uses only standard `vkQueuePresentKHR`. Normal production configurations and artifacts remain OpenGL-only. |
