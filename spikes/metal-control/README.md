...
`nextDrawable` never runs on the Java frame caller. Each renderer owns one serial native queue, permits at most one pending acquisition and one retained ready drawable, and schedules another request only when needed. `render` consumes a ready drawable or records one skipped attempt and returns; it has no retry loop or process-global worker. Close invalidates the acquisition generation and drains a pending request, bounded by `allowsNextDrawableTimeout=YES`, before releasing its queue.
